...
const ItemPopup: React.FC<ItemPopupProps> = ({ item, popup, setPopup }) => {
  ...
  return (
    <>
      <div className={`item__popup  ${ popup ? "active" : "" } ${ lightsOn ? "" : "dark"}`}>
        <div className="item__popup--grid">
          ...
          <div className="item__popup--info">
            <div className="item__popup--info-top">
              ...
              <button className={`item__popup--close-button  ${ lightsOn ? "" : "dark"}`} onClick={()=>setPopup(false)}>X</button>
            </div>
            <div>{`$${item.price}`}</div>
      ...
      <div className={`item__popup--black-screen ${ popup ? "active" : "" }`} onClick={()=> setPopup(false)}/>
    </>
  )
};
